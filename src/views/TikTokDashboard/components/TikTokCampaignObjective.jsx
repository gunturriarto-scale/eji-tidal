...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="glass-panel" style={{ padding: '0.75rem 1rem', border: '1px solid rgba(255,0,80,0.25)', borderRadius: '8px' }}>
      ...
      <p style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)' }}>Impressions: {fmt(d.impressions)}</p>
    </div>
  );
};
...
export const TikTokCampaignObjective = ({ data }) => {
    ...
    <div>
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
          ...
          <XAxis dataKey="objective" tick={{ fontSize: 10, fill: 'var(--text-tertiary)' }} />
          ...
          <Bar dataKey="spend" radius={[4, 4, 0, 0]}>
            {data.map((entry, i) => (
              <Cell key={i} fill={CHANNEL_COLORS[entry.objective] || '#FF0050'} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
